...
export class DriversComponent implements OnInit {
  ...
  getAllProcessData(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.http
          .post<any>(
            this.url + '/get-transportistas-datatable',
  ...
  onUpdate(form: any, id: any) {
    ...
    const { name, lastName, capacity, phone, experience } = this.editForm.value;
    ...
    this.updateObj.capacidad = capacity;
    this.updateObj.telefono = phone;
    this.updateObj.experiencia_laboral = experience;

    this.adminService.updateDriver(this.updateObj).subscribe(
      (result) => {
  ...
  deleteUset(id: any) {
    ...
    this.adminService.deleteDriver(this.dataDeleteObj).subscribe(
      (result) => {
        if (result.resultado == true) {
          this.showSuccess('Transportista eliminado.', 'Listo');
          this.reloadTable();
        } else {
          this.showAlert(result.mensaje, 'Error');
        }
